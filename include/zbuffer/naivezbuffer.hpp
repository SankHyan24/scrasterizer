...
class ZBuffer // Naive Z-Buffer Rasterizer
{
public:
    ...
    void drawPrepare(OBJ &obj, Camera &camera)
    {
...
private:
    void __vertexShader(OBJ &obj, Camera &camera)
    {
        ...
        for (int face_index = 0; face_index < faces_.size(); face_index++)
        {
            ...
            if (__triangleInScreen(triangle))
    ...
    {
        {
            ...
            if (count++ != 0)
                return;
            std::fstream file("vertices.txt", std::ios::out);
            for (auto &v : vertices)
                file << v.nx << " " << v.ny << " " << v.nz << std::endl;
        }
    }

    bool __insideTriangle(int x, int y, const Triangle &triangle)
    {
    ...
    void __drawTriangle(const Triangle &triangle)
    {
        ...
        for (int x = xStart; x <= xEnd; x++)
            for (int y = yStart; y <= yEnd; y++)
                if (__insideTriangle(x, y, triangle))
                {
                    float lambda1, lambda2, lambda3;
                    ComputeBarycentricCoords(x, y, triangle, lambda2, lambda3);
    ...
    bool __triangleInScreen(const Triangle &triangle)
    {
        if (!use_cull_face)
            return true;
        // cull face
    ...
    void ComputeBarycentricCoords(int x, int y, const Triangle &triangle, float &lambda1, float &lambda2)
    {
        // barycentric coordinate
        auto &v0 = vertices[triangle.v0];
        auto &v1 = vertices[triangle.v1];
    ...
    int obj_face_offset{0};   // for each obj, the offset of faces
};
...
public:
    ...
    void render() override
    {
        ...
        for (auto &obj : scene->objs)
            zbuffer->drawPrepare(*obj, scene->getCameraV());
        zbuffer->draw();
    ...
    {
        assert(false && "NaiveZBufferRaster GPU Not Implemented!");
        _autoRotateCamera();
        {
        }
        scene->drawGPU();
...
private:
    void __putColorBuffer2TextureMap()
    {
        ...
        //
        // SCRA::Utils::saveAsPPM(colorPrecompute, width, height, "naivezbuffer.ppm");
    }
